...
def py_trees_run(
    ...
    children = (
        RunThenSuccess(f"child {i}")
        for i in range(length - 1)
    )
    root = py_trees.composites.Sequence(
        f"root", memory=True, children=list(children)
    )
...
def btreeny_run(
    ...
    action = bt.sequential(*(sa.run_then_ok() for _ in range(length)))
...
__benchmarks__ = [
    (
        functools.partial(py_trees_run, length=1),
        functools.partial(btreeny_run, length=1),
        "Sequential (length 1)",
    ),
    (
